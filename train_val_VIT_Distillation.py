...
def cal_anomaly_map_(fs_list, ft_list, img, mean, std, out_size=224, amap_mode='mul'):
    """
    Calculate and plot the aggregated anomaly map given lists of features,
    using MSE loss to compute the anomaly score per patch.
    
    Args:
        fs_list (list of torch.Tensor): List of source features with shape (B, num_blocks, patches, latent_dim).
        ft_list (list of torch.Tensor): List of target features with shape (B, num_blocks, patches, latent_dim).
        img (torch.Tensor): Input image tensor with shape (B, C, H, W).
        out_size (int): Output size for the anomaly map (default: 224).
        amap_mode (str): How to aggregate anomaly maps; 'mul' for multiplication, any other for addition.
    
    Returns:
        anomaly_map (np.ndarray): Aggregated anomaly map of shape (out_size, out_size).
        a_map_list (list of np.ndarray): List of individual anomaly maps.
    """
    # Initialize aggregated anomaly map using NumPy.
    anomaly_map = np.ones((out_size, out_size)) if amap_mode == 'mul' else np.zeros((out_size, out_size))
    a_map_list = []

    # Iterate over feature pairs.
    for fs, ft in zip(fs_list, ft_list):
        # Expected shape for fs,ft: (B, patches, latent_dim)
        # Compute MSE per patch along the latent dimension.
        # Compute squared difference and then average along latent dimension:
        # Resulting shape: (B, patches)
        a_map = 1 - F.cosine_similarity(fs[:, 2:], ft[:, 2:], dim=-1)
        #a_map = ((fs[:,2:] - ft[:,2:]) ** 2).mean(dim=-1)
        # Alternatively, you can use F.mse_loss with reduction='none':
        # a_map = F.mse_loss(fs, ft, reduction='none').mean(dim=-1)
        ...
        # Reshape to 2D map: (B, 1, grid_size, grid_size)
        a_map = a_map.view(B, 1, grid_size, grid_size)
        
        # Upsample to desired output size: (B, 1, out_size, out_size)
        a_map = F.interpolate(a_map, size=out_size, mode='bilinear', align_corners=True)
        
        # Extract the first (and assumed only) image in the batch and convert to NumPy.
        a_map_np = a_map[0, 0, :, :].cpu().detach().numpy()
        a_map_list.append(a_map_np)
    ...
    # Post-process the aggregated map.
    anomaly_map = gaussian_filter(anomaly_map, sigma=4)
    norm_map = min_max_norm(anomaly_map)  # Ensure min_max_norm is defined elsewhere.
    heatmap = norm_map#cvt2heatmap(norm_map * 255)  # Ensure cvt2heatmap is defined elsewhere.
    
    
    img_np = img.detach().cpu().squeeze(0) * torch.tensor(std).view(3, 1, 1) + torch.tensor(mean).view(3, 1, 1)
    img_np = img_np.clamp(0, 1).permute(1, 2, 0).numpy()

    # Plot the input image and anomaly map.
    plt.figure(figsize=(12, 6))
    ...
    plt.subplot(1, 2, 2)
    # Option: Display the heatmapped anomaly map, or simply the raw anomaly map with a colormap.
